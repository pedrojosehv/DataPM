#!/usr/bin/env python3
"""
Final Deduplication Processor for DataPM
Uses correct column names and provides comprehensive duplicate analysis
"""
...
class FinalDeduplicationProcessor:
    """Final deduplication processor for DataPM CSV files"""
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(f'final_deduplication_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
    ...
    def analyze_cross_file_duplicates(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Analyze duplicates across all files using correct column names"""
        ...
        # Check for duplicates based on key combinations using correct column names
        key_combinations = [
            ['Job title (original)', 'Company'],
            ['Job title (original)', 'Company', 'City'],
            ['Job title (original)', 'Company', 'Seniority'],
            ['Company', 'City', 'Seniority'],
            ['Job title (original)', 'Company', 'Country'],
            ['Job title (original)', 'Company', 'State']
        ]
        ...
        for combo in key_combinations:
            ...
            if len(available_fields) >= 2:
                ...
                if len(duplicate_combinations) > 0:
                    analysis['duplicate_analysis'][f"duplicates_by_{'_'.join([f.replace(' ', '_') for f in available_fields])}"] = {
                    ...
                    for combo_values, count in duplicate_combinations.items():
                        ...
                        display_columns = ['_source_file', 'Job title (original)', 'Company', 'City', 'Country', 'Seniority']
                        available_display_columns = [col for col in display_columns if col in duplicate_records.columns]
                        
                        analysis['cross_file_duplicates'].append({
                            'fields': available_fields,
                            'values': combo_dict,
                            'count': count,
                            'records': duplicate_records[available_display_columns].to_dict('records')
                        })
    ...
    def find_similar_jobs(self, df: pd.DataFrame, similarity_threshold: float = 0.8) -> List[Dict[str, Any]]:
        ...
        if 'Company' not in df.columns:
            self.logger.warning("Company column not found, skipping similar jobs analysis")
            return similar_jobs
                    ...
                    if similarity_score >= similarity_threshold:
                        similar_jobs.append({
                            'record1': {
                                ...
                                'title': record1.get('Job title (original)', 'Unknown'),
                                'location': record1.get('City', 'Unknown'),
                                'seniority': record1.get('Seniority', 'Unknown')
                            },
                            'record2': {
                                'file': record2['_source_file'],
                                'title': record2.get('Job title (original)', 'Unknown'),
                                'location': record2.get('City', 'Unknown'),
    ...
    def calculate_job_similarity(self, record1: pd.Series, record2: pd.Series) -> float:
        ...
        # Compare job titles
        title1 = str(record1.get('Job title (original)', '')).lower().strip()
        title2 = str(record2.get('Job title (original)', '')).lower().strip()
        ...
        location_match = (str(record1.get('City', '')) == str(record2.get('City', '')))
    ...
    def process_all_files(self, output_dir: str = None) -> Dict[str, Any]:
        """Process all CSV files for final deduplication"""
        ...
        self.generate_final_report(results, analysis, similar_jobs, output_path)
        
        return results
    
    def generate_final_report(self, results: Dict[str, Any], analysis: Dict[str, Any], 
                            similar_jobs: List[Dict[str, Any]], output_dir: Path):
        """Generate a detailed report of the final deduplication process"""
        
        report_file = output_dir / "final_deduplication_report.txt"
        
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 70 + "\n")
            f.write("DATAPM FINAL DEDUPLICATION ANALYSIS REPORT\n")
            f.write("=" * 70 + "\n")
            ...
            for i, duplicate_group in enumerate(analysis['cross_file_duplicates'], 1):
                ...
                for record in duplicate_group['records']:
                    f.write(f"    - {record['_source_file']}: {record.get('Job title (original)', 'Unknown')} at {record.get('Company', 'Unknown')}\n")
...
def main():
    """Main function to run the final deduplication process"""
    
    processor = FinalDeduplicationProcessor()
    
    print("🧹 DataPM Final Deduplication Processor")
    print("=" * 60)
    ...
    if 'error' not in results:
        ...
        print(f"📁 Output directory: {processor.csv_processed_dir.parent}/csv_duplicates/final_deduplicated")
        print(f"📊 Summary:")
